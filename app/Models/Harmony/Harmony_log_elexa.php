...
class Harmony_log_elexa extends Model
{
    use HasFactory;
            ...
            foreach ($changes as $column => $value) {
                ...
                if ($column == 'receive_id')
                    ...
                    if ($column == 'Hotel Bank Transfer') {
                        $Famount = Harmony_SMS_alerts::where('id', $value)->sum('amount');
                        ...
                        if ($type == 'edit') {
                            foreach ($value as $key => $item) {
                                if (!in_array($item, $product['receive_id'])) {
                                    $check_detail = Harmony_revenue_credit::where('id', $item)->where('status', 8)
