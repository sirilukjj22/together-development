...
class ReportAgodaAccountReceivableController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $data_query = SMS_alerts::whereBetween('date', [date('Y-m-d 21:00:00', strtotime('last day of previous month')), date('Y-m-t 20:59:59')])
            ->where('status', 5)->where('status_receive_agoda', 1)->whereNull('date_into')
        ...
        return view('report.agoda_account_receivable.index', compact('data_query', 'total_sms_amount', 'filter_by', 'search_date'));
    }
    ...
    public function search(Request $request)
    {
        ...
        $query = SMS_alerts::query()->where('status_receive_agoda', 1)->whereNull('date_into');
        ...
        if ($filter_by == "date") {
            ...
            $query->orWhereBetween(DB::raw('DATE(date_into)'), [$startDate, $endDate])->where('status', 5)->where('status_receive_agoda', 1);
        ...
        if ($filter_by == "month") {
            ...
            $query->orWhereBetween(DB::raw('DATE(date_into)'), [date($startDate.'-01'), date('Y-m-t', strtotime("$startDate-01"))])->where('status', 5)->where('status_receive_agoda', 1);
        ...
        if ($filter_by == "year") {
            ...
            $query->orWhereYear(DB::raw('DATE(date_into)'), $startDate)->where('status', 5)->where('status_receive_agoda', 1);
        ...
        if ($request->method_name == "search") {
            return view('report.agoda_account_receivable.index', compact('data_query', 'total_sms_amount', 'filter_by', 'search_date', 'startDate'));
            ...
            $pdf = FacadePdf::loadView('pdf.report_agoda.agoda_account_receivable.1A', compact('data_query', 'total_sms_amount', 'filter_by', 'search_date', 'startDate', 'page_item'));
            return $pdf->stream();

        } elseif ($request->method_name == "excel") {
            return Excel::download(new DebtorAgodaAccountReceivableExport($filter_by, $data_query, $total_sms_amount, $search_date), 'agoda_account_receivable.xlsx', \Maatwebsite\Excel\Excel::XLSX);
        }
    }
}
