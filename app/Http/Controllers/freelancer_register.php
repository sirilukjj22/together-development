...
class freelancer_register extends Controller
{
    ...
    public function save(Request $request)
    {
        try {
            ...
            return redirect()->route('freelancer.index')->with('success', 'บันทึกข้อมูลเรียบร้อย');
        } catch (\Throwable $e) {
            return response()->json([
                'error' => $e->getMessage()
            ], 500);
    ...
    public function changeStatus($id)
    {
        ...
        return redirect()->route('freelancer_checked.index')->with('success', 'บันทึกข้อมูลเรียบร้อย');
    }
    public function ac(Request $request)
    {
        $ac = $request->value;
        if ($ac == 1 ) {
            $query = Freelancer_checked::query();
    ...
    public function ap(Request $request)
    {
        ...
        if ($ac == 2 ) {
            $query = Freelancer_checked::query();
            $Freelancer_checked = $query->where('status', '2')->get();
        }
        $Mbooking = master_document::select('name_en','id')->get();
        return view('freelancer_checked.index',compact('Freelancer_checked','Mbooking'));
    }
    public function Approve($id){
        $query = Freelancer_checked::find($id);
        $query->status = 2;
        $query->save();
        if ($Freelancer_checked->status == 2) {
