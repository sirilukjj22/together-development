...
use App\Exports\DebtorElexaOutstandingExport;
...
class ReportElexaOutstandingController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('report.elexa_outstanding.index', compact('data_query', 'total_elexa_amount', 'filter_by', 'search_date'));
    }
    ...
    public function search(Request $request)
    {
        ...
        if ($request->method_name == "search") {
            return view('report.elexa_outstanding.index', compact('data_query', 'total_elexa_amount', 'filter_by', 'search_date', 'startDate'));
            ...
            $pdf = FacadePdf::loadView('pdf.report_elexa.elexa_outstanding.1A', compact('data_query', 'total_elexa_amount', 'filter_by', 'search_date', 'startDate', 'page_item'));
            return $pdf->stream();

        } elseif ($request->method_name == "excel") {
            return Excel::download(new DebtorElexaOutstandingExport($filter_by, $data_query, $total_elexa_amount, $search_date), 'elexa_outstanding.xlsx', \Maatwebsite\Excel\Excel::XLSX);
        }
    }
}
