<?php
    ...
    //Billing
    function showingEntriesTableBilling($data, $table)
    {
        $total = $data->total();
        $currentPage = $data->currentPage();
        $perPage = !empty($_GET['table']) && @$_GET['table'] == $table ? $_GET['perPage'] : 10;
    ...
    function paginateTableBilling($data, $table)
    {
        $currentPage = 1;
        $perPage = !empty($_GET['perPage']) ? $_GET['perPage'] : 10;
        $num = 0;
        ...
        $html .= '<div class="pagination" style="white-space: nowrap;">
                    <a href="#" onclick="getPageBilling('.($currentPage == 1 ? 1 : $currentPage - 1).', '.$perPage.', '."'$table'".')" class="r-l-md">&laquo;</a>';
                    if ($data->total() > 0) {
                        if ($currentPage > 3)
                        {
                        ...
                        if ($currentPage < $data->lastPage() - 2)
                        {
                            ...
                            $html .= '<a href="#" onclick="getPageBilling(' .$data->lastPage(). ', ' . $perPage . ', '."'$table'".')">'.$data->lastPage().'</a>';
                        }
                    }
          $html .= '<a href="#" onclick="getPageBilling('.($data->total() > 10 ? $currentPage + 1 : 1).', '.$perPage.', '."'$table'".')" class="r-r-md">&raquo;</a>
                  </div>';
