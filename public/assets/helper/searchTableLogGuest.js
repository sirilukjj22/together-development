...
function getPageVisit(page, perPage, table_n)
{
    ...
    if (table_n == "guest-Visit") {
        ...
        var table = $('#' + table_name).dataTable({
            ...
            info: false,
            ajax: {
                url: '/Visit-guest-paginate-table',
                type: 'POST',
                dataType: "json",
                cache: false,
                data: {
...
function showingEntriesSearchBilling($page, $total, $table_name)
{
    var total = $total;
    var currentPage = parseInt($page);
    var perPage = parseInt($('#search-per-page-' + $table_name).val());
...
// Paginate
function paginateSearchBilling($total, $table, $link)
{
    var total = $total;
    var currentPage = parseInt($('#currentPage-' + $table).val());
    var perPage = parseInt($('#search-per-page-' + $table).val());
    ...
    html += '<div class="pagination" style="white-space: nowrap;">';
    html += '<a href="#" class="r-l-md" onclick="getPageBilling(' + previousPageUrl + ', ' + perPage + ', ' + "'" + table + "'" + ')">&laquo;</a>';
    if (total > 0) {
        if (currentPage > 3)
        {
            html += '<a class="" href="#" onclick="getPageBilling(1, ' + perPage + ', ' + "'" + table + "'" + ')">1</a>';
        ...
        for ($i = Math.max(1, currentPage - 2); $i <= Math.min(Math.ceil(total / perPage), currentPage + 2); $i++)
        {
            ...
            if ($i == currentPage)
            {
                html += '<a class="active" href="#" onclick="getPageBilling(' + $i + ', ' + perPage + ', ' + "'" + table + "'" + ')">' + $i + '</a>';
            } else {
                html += '<a class="" href="#" onclick="getPageBilling(' + $i + ', ' + perPage + ', ' + "'" + table + "'" + ')">'+ $i +'</a>';
            }
        }
        ...
        if (currentPage < Math.ceil(total / perPage) - 2)
        {
            ...
            html += '<a href="#" onclick="getPageBilling(' + Math.ceil(total / perPage) + ', ' + perPage + ', ' + "'" + table + "'" + ')">'+ Math.ceil(total / perPage) +'</a>';
        }
    }
    html += '<a href="#" class="r-r-md" onclick="getPageBilling(' + nextPageUrl + ', ' + perPage + ', ' + "'" + table + "'" + ')">&raquo;</a>';
    html += '</div>';

    return html;
}

function getPageBilling(page, perPage, table_n)
{
    var table_name = table_n + 'Table';
    var getUrl = window.location.pathname;
    var type = $('#status').val();
    ...
    if (table_n == "guest-Billing") {
        ...
        var table = $('#' + table_name).dataTable({
            ...
            info: false,
            ajax: {
                url: '/Billing-guest-paginate-table',
                type: 'POST',
                dataType: "json",
                cache: false,
                data: {
    ...
    $('#' + table_n + '-paginate').children().remove().end();
    $('#' + table_n + '-showingEntries').text(showingEntriesSearchBilling(page, total, table_n));
    $('#' + table_n + '-paginate').append(paginateSearchBilling(total, table_n, getUrl));
