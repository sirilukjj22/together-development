...
function getGraphMonthSales(typeRevenue) {
  ...
  $.ajax({
      type: "GET",
      url: "/harmony-debtor-elexa-graph-month-sales",
      datatype: "JSON",
      async: false,
...
function getGraphMonthCharge(year, typeRevenue) {
  ...
  $.ajax({
      type: "GET",
      url: "/harmony-debtor-elexa-graph-month-charge",
      datatype: "JSON",
      async: false,
      success: function(response) {
          // Sample data for watch revenue over 30 days
...
function updateSummaryTotals() {
  ...
  document.querySelector("#agodaCharge span:nth-child(2)").textContent = formatNumberCol(parseFloat(inputTotalElexaCharge).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ","));
