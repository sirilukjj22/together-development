    ...
    <tbody>
        @foreach ($data_query as $key => $item)
            <tr style="vertical-align: middle;">
                ...
                <td style="text-align:center;">{{ $item->Invoice_ID}}</td>
                <td style="text-align:center;">{{ $item->Quotation_ID}}</td>
                @if ($item->type_Proposal == 'Company')
                    <td style="text-align:left;">{{ @$item->company00->Company_Name}}</td>
                @else
                ...
                <td style="text-align:right;">{{ number_format($item->sumpayment, 2) }}</td>
            </tr>
        @endforeach
        <tr style="font-weight: bold;">
            <td colspan="7" style="text-align: center;">Total</td>
            <td style="text-align: right;">{{ number_format($total_invoice, 2) }}</td>
            <td></td>
        </tr>
    </tbody>
</table>
