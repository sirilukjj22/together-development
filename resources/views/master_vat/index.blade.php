...
@section('content')
            ...
            <div class="row align-items-center">
                ...
                <div class="col-auto">
                    <button type="button" class="btn btn-color-green lift btn_modal" data-bs-toggle="modal" data-bs-target="#EventVat">
                        <i class="fa fa-plus"></i> เพิ่มภาษีมูลค่าเพิ่ม</button>
                </div>
                <!-- Prename Modal Center-->
                <div class="modal fade" id="EventVat" tabindex="-1" aria-labelledby="PrenameModalCenterTitle"
                style="display: none;" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="PrenameModalCenterTitle">เพิ่มภาษีมูลค่าเพิ่ม</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="col-12">
                                        <div class="card-body">
                                            <form action="{{route('Mvat.save')}}" method="POST" enctype="multipart/form-data" class="row g-3 basic-form" id="form-id">
                                                @csrf
                ...
                <div class="col-md-12 col-12">
                    <div class="card mb-3">
                        <div class="card-body">
                                    ...
                                    <div class="top-table-3c_1">
                                        ...
                                        <i class="fas fa-angle-down arrow-dropdown"></i>
                                            ...
                                            <ul class="dropdown-menu border-0 shadow p-3">
                                                <li><a class="dropdown-item py-2 rounded" href="{{ url('Mvat', 'Mvat.all') }}">All</a></li>
                                                <li><a class="dropdown-item py-2 rounded" href="{{ url('Mvat', 'Mvat.ac') }}">Active</a></li>
                                                <li><a class="dropdown-item py-2 rounded" href="{{ url('Mvat', 'Mvat.no') }}">Disabled</a></li>
                                            </ul>
                                        </div>
                                        <button type="button" class="btn btn-color-green lift btn_modal" onclick="window.location.href='{{ route('Mvat.Log') }}'">
                                            LOG
                                        </button>
                                    </div>

                                    <label class="entriespage-label">entries per page :</label>
                                    <select class="entriespage-button" id="search-per-page-Mvat" onchange="getPage(1, this.value, 'Mvat')"> <!-- ชือนำหน้าตาราง, ชื่อ Route -->
                                        <option value="10" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 10 && @$_GET['table'] == "Mvat" ? 'selected' : '' }}>10</option>
                                        <option value="25" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 25 && @$_GET['table'] == "Mvat" ? 'selected' : '' }}>25</option>
                                        <option value="50" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 50 && @$_GET['table'] == "Mvat" ? 'selected' : '' }}>50</option>
                                        <option value="100" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 100 && @$_GET['table'] == "Mvat" ? 'selected' : '' }}>100</option>
                                    </select>
                                    <input class="search-button search-data" id="Mvat" style="text-align:left;" placeholder="Search" />

                                </div>
                            </caption>
                            <div style="min-height: 70vh;" class="mt-2">
                                <table id="MvatTable" class="example ui striped table nowrap unstackable hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td style="text-align: center;">
                                                    <div class="btn-group">
                                                        ...
                                                        <ul class="dropdown-menu border-0 shadow p-3">
                                                            <li><a class="dropdown-item py-2 rounded" href="#" onclick="view_detail({{$item->id}})" href="#" data-bs-toggle="modal" data-bs-target="#EventVat">View</a></li>
                                                            <li><a class="dropdown-item py-2 rounded" id="btn-edit" onclick="edit({{$item->id}})" href="#" data-bs-toggle="modal" data-bs-target="#EventVat">Edit</a></li>
                                                        </ul>
                                                    </div>
                                                </td>
                                            </tr>
                            ...
                            <input type="hidden" id="get-total-Mvat" value="{{ $Mvat->total() }}">
                            <input type="hidden" id="currentPage-Mvat" value="1">
                            <caption class="caption-bottom">
                                <div class="md-flex-bt-i-c">
                                    <p class="py2" id="Mvat-showingEntries">{{ showingEntriesTable($Mvat, 'Mvat') }}</p>
                                        <div id="Mvat-paginate">
                                            {!! paginateTable($Mvat, 'Mvat') !!} <!-- ข้อมูล, ชื่อตาราง -->
                                        </div>
                                </div>
                            </caption>
                        </div>
    ...
    <script type="text/javascript" src="{{ asset('assets/helper/searchTableMasterVat.js')}}"></script>
    <script>
    ...
    <script>
        ...
        $('.btn_modal').on('click', function() {
            ...
            $('#comment').text('');
            $('#search_list').text('');
            $('#PrenameModalCenterTitle').text("เพิ่มภาษีมูลค่าเพิ่ม");
        ...
        $('#name_th').on('keyup', function () {
            ...
            $.ajax({
                type:   "GET",
                url:    "{!! url('/Mvat/search-list2/"+datakey+"') !!}",
                datatype:   "JSON",
                success: function(data) {
                    if (data.name_th) {
                        console.log(data.name_th);
                        $('#comment').text("** '" + data.name_th + "' มีอยูในระบบแล้ว !");
                        $('#search_list').text('มีภาษีมูลค่าเพิ่มซ้ำกันแล้ว');
        ...
        function edit(id) {
            ...
            document.getElementById('btn-save').disabled = false;
            $('#comment').text('');
            $('#PrenameModalCenterTitle').text("แก้ไขภาษีมูลค่าเพิ่ม");

            jQuery.ajax({
                type:   "GET",
                url:    "{!! url('/Mvat/edit/"+id+"') !!}",
                datatype:   "JSON",
                async:  false,
        ...
        function view_detail(id) {
            ...
            $('#PrenameModalCenterTitle').text("รายละเอียดภาษีมูลค่าเพิ่ม");
        ...
        $('#btn-save').on('click', function() {
            ...
            if (name_th != '') {
                ...
                if (module_name == "create") {
                    ...
                    jQuery.ajax({
                    type:   "GET",
                    url:    "{!! url('/Mvat/check-edit-name/"+id+"/"+datakey+"') !!}",
                    datatype:   "JSON",
                    async:  false,
                    success: function(result) {
                            if (result.data.name_th == datakey && result.data.name_en == dataEN) {
                                ...
                                if (module_name == "edit") {
                                    jQuery.ajax({
                                    type:   "GET",
                                    url:    "{!! url('/Mvat/update/"+id+"/"+datakey+"/"+dataEN+"') !!}",
                                    datatype:   "JSON",
                                    async:  false,
