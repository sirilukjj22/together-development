...
@section('content')
        ...
        <div class="container-xl">
            <div class="row align-items-center">
                <div class="col sms-header">
                    <small class="text-muted">Welcome to Quantity.</small>
                    <div class=""><span class="span1">Quantity (ปริมาณ)</span></div>
                </div>
                <div class="col-auto">
                    <button type="button" class="btn btn-color-green lift btn_modal" data-bs-toggle="modal" data-bs-target="#QuantityCreate">
                        <i class="fa fa-plus"></i> เพิ่มปริมาณ</button>
                </div>
                <!-- Prename Modal Center-->
                <div class="modal fade" id="QuantityCreate" tabindex="-1" aria-labelledby="PrenameModalCenterTitle"
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <div class="col-12">
                                        <div class="card-body">
                                            <form action="{{ route('Mproduct.save.quantity') }}" method="POST" enctype="multipart/form-data" class="row g-3 basic-form" id="form-id">
                                                @csrf
                ...
                <div class="col-md-12 col-12">
                    <div class="card mb-3">
                        <div class="card-body">
                                    ...
                                    <div class="top-table-3c_1">
                                        <div class="dropdown">
                                            <button class="bd-button statusbtn enteriespage-button" style="min-width: 100px; text-align: left;" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false" style="text-align: left;">
                                                @if ($menu == 'Quantity.all')
                                                    All
                                                @elseif ($menu == 'Quantity.ac')
                                                    Active
                                                @elseif ($menu == 'Quantity.no')
                                                    Disabled
                                                @else
                                                    Status
                                                @endif
                                        <i class="fas fa-angle-down arrow-dropdown"></i>
                                            ...
                                            <ul class="dropdown-menu border-0 shadow p-3">
                                                <li><a class="dropdown-item py-2 rounded" href="{{ url('/Mproduct/Quantity', 'Quantity.all') }}">All</a></li>
                                                <li><a class="dropdown-item py-2 rounded" href="{{ url('/Mproduct/Quantity', 'Quantity.ac') }}">Active</a></li>
                                                <li><a class="dropdown-item py-2 rounded" href="{{ url('/Mproduct/Quantity', 'Quantity.no') }}">Disabled</a></li>
                                            </ul>
                                        </div>
                                        <button type="button" class="btn btn-color-green lift btn_modal" onclick="window.location.href='{{ route('Quantity.Log') }}'">
                                            LOG
                                        </button>
                                    </div>

                                    <label class="entriespage-label">entries per page :</label>
                                    <select class="entriespage-button" id="search-per-page-quantity" onchange="getPage(1, this.value, 'quantity')"> <!-- ชือนำหน้าตาราง, ชื่อ Route -->
                                        <option value="10" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 10 && @$_GET['table'] == "quantity" ? 'selected' : '' }}>10</option>
                                        <option value="25" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 25 && @$_GET['table'] == "quantity" ? 'selected' : '' }}>25</option>
                                        <option value="50" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 50 && @$_GET['table'] == "quantity" ? 'selected' : '' }}>50</option>
                                        <option value="100" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 100 && @$_GET['table'] == "quantity" ? 'selected' : '' }}>100</option>
                                    </select>
                                    <input class="search-button search-data" id="quantity" style="text-align:left;" placeholder="Search" />

                                </div>
                            </caption>
                            <div style="min-height: 70vh;" class="mt-2">
                                <table id="quantityTable" class="example ui striped table nowrap unstackable hover">
                                    <thead>
                                        <tr>
                            ...
                            <input type="hidden" id="get-total-quantity" value="{{ $quantity->total() }}">
                            <input type="hidden" id="currentPage-quantity" value="1">
                            <caption class="caption-bottom">
                                <div class="md-flex-bt-i-c">
                                    <p class="py2" id="quantity-showingEntries">{{ showingEntriesTable($quantity, 'quantity') }}</p>
                                        <div id="quantity-paginate">
                                            {!! paginateTable($quantity, 'quantity') !!} <!-- ข้อมูล, ชื่อตาราง -->
                                        </div>
                                </div>
                            </caption>
                        </div>
    ...
    <script type="text/javascript" src="{{ asset('assets/helper/searchTableMasterQuantity.js')}}"></script>
    <script>
    ...
    <script>
        ...
        $('.btn_modal').on('click', function() {
            ...
            $('#comment').text('');
            $('#search_list').text('');
            $('#PrenameModalCenterTitle').text("เพิ่มปริมาณ");
        ...
        function edit(id) {
            ...
            document.getElementById('btn-save').disabled = false;
            $('#comment').text('');
            $('#PrenameModalCenterTitle').text("แก้ไขปริมาณ");

            jQuery.ajax({
                type:   "GET",
                url:    "{!! url('/Mproduct/Quantity/edit/"+id+"') !!}",
                datatype:   "JSON",
                async:  false,
        ...
        function view_detail(id) {
            ...
            $('#PrenameModalCenterTitle').text("รายละเอียดปริมาณ");
        ...
        $('#btn-save').on('click', function() {
            ...
            if (name_th != '') {
                ...
                if (module_name == "create") {
                    ...
                    jQuery.ajax({
                    type:   "GET",
                    url:    "{!! url('/Mproduct/quantity/check-edit-name/"+id+"/"+datakey+"') !!}",
                    datatype:   "JSON",
                    async:  false,
                    success: function(result) {
                            if (result.data.name_th == datakey && result.data.name_en == dataEN) {
                                ...
                                if (module_name == "edit") {
                                    jQuery.ajax({
                                    type:   "GET",
                                    url:    "{!! url('/Mproduct/quantity/update/"+id+"/"+datakey+"/"+dataEN+"') !!}",
                                    datatype:   "JSON",
                                    async:  false,
        ...
        function btnstatus(id) {
            jQuery.ajax({
                type: "GET",
                url: "{!! url('/Mproduct/changeStatus_quantity/" + id + "') !!}",
                datatype: "JSON",
                async: false,
                success: function(result) {
