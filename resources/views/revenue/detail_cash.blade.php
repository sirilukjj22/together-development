...
@section('content')
                    ...
                    <div class="card p-4 mb-4">
                        <caption class="caption-top">
                            <div>
                                <div class="flex-end-g2">
                                    ...
                                    <select class="entriespage-button" id="search-per-page-revenueCash" onchange="getPage(1, this.value, 'revenueCash')"> <!-- ชือนำหน้าตาราง, ชื่อ Route -->
                                        <option value="10" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 10 && @$_GET['table'] == "revenueCash" ? 'selected' : '' }}>10</option>
                                        <option value="25" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 25 && @$_GET['table'] == "revenueCash" ? 'selected' : '' }}>25</option>
                                        <option value="50" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 50 && @$_GET['table'] == "revenueCash" ? 'selected' : '' }}>50</option>
                                        <option value="100" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 100 && @$_GET['table'] == "revenueCash" ? 'selected' : '' }}>100</option>
                                    </select>
                                    <input class="search-button search-data" id="revenueCash" style="text-align:left;" placeholder="Search" />
                                </div>
                        </caption>
                        <div style="min-height: 70vh;">
                            <table id="revenueCashTable" class="example ui striped table nowrap unstackable hover">
                                <thead>
                                    <tr>
                        ...
                        <caption class="caption-bottom">
                            <div class="md-flex-bt-i-c">
                                <p class="py2" id="revenueCash-showingEntries">{{ showingEntriesTable($data_query, 'revenueCash') }}</p>
                                <div class="font-bold ">ยอดรวมทั้งหมด {{ number_format($total_query, 2) }} บาท</div>
                                    <div id="revenueCash-paginate">
                                        {!! paginateTable($data_query, 'revenueCash') !!} <!-- ข้อมูล, ชื่อตาราง -->
                                    </div>
                            </div>
                        </caption>
                    </div> <!-- .card end -->
    ...
    <input type="hidden" id="get-total-revenueCash" value="{{ $data_query->total() }}">
    <input type="hidden" id="currentPage-revenueCash" value="1">
    ...
    <script type="text/javascript" src="{{ asset('assets/helper/searchTableRevenueDetailCash.js')}}"></script>
