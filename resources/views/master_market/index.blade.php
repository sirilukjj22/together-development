...
@section('content')
                ...
                <div class="col-md-12 col-12">
                    <div class="card mb-3">
                        <div class="card-body">
                                    ...
                                    <div class="top-table-3c_1">
                                        <div class="dropdown">
                                            <button class="bd-button statusbtn enteriespage-button" style="min-width: 100px; text-align: left;" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false" style="text-align: left;">
                                                @if ($menu == 'Mmarket.all')
                                                    All
                                                @elseif ($menu == 'Mmarket.ac')
                                                    Active
                                                @elseif ($menu == 'Mmarket.no')
                                                    Disabled
                                                @else
                                                    Status
                                                @endif
                                        <i class="fas fa-angle-down arrow-dropdown"></i>
                                            ...
                                            <ul class="dropdown-menu border-0 shadow p-3">
                                                <li><a class="dropdown-item py-2 rounded" href="{{ url('Mmarket', 'Mmarket.all') }}">All</a></li>
                                                <li><a class="dropdown-item py-2 rounded" href="{{ url('Mmarket', 'Mmarket.ac') }}">Active</a></li>
                                                <li><a class="dropdown-item py-2 rounded" href="{{ url('Mmarket', 'Mmarket.no') }}">Disabled</a></li>
                                            </ul>
                                        </div>
                                        <button type="button" class="btn btn-color-green lift btn_modal" onclick="window.location.href='{{ route('Mmarket.Log') }}'">
                                            LOG
                                        </button>
                                    </div>

                                    <label class="entriespage-label">entries per page :</label>
                                    <select class="entriespage-button" id="search-per-page-Mmarket" onchange="getPage(1, this.value, 'Mmarket')"> <!-- ชือนำหน้าตาราง, ชื่อ Route -->
                                        <option value="10" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 10 && @$_GET['table'] == "Mmarket" ? 'selected' : '' }}>10</option>
                                        <option value="25" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 25 && @$_GET['table'] == "Mmarket" ? 'selected' : '' }}>25</option>
                                        <option value="50" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 50 && @$_GET['table'] == "Mmarket" ? 'selected' : '' }}>50</option>
                                        <option value="100" class="bg-[#f7fffc] text-[#2C7F7A]" {{ !empty(@$_GET['perPage']) && @$_GET['perPage'] == 100 && @$_GET['table'] == "Mmarket" ? 'selected' : '' }}>100</option>
                                    </select>
                                    <input class="search-button search-data" id="Mmarket" style="text-align:left;" placeholder="Search" />

                                </div>
                            </caption>
                            <div style="min-height: 70vh;" class="mt-2">
                                <table id="MmarketTable" class="example ui striped table nowrap unstackable hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td style="text-align: center;">
                                                    <div class="btn-group">
                                                        ...
                                                        <ul class="dropdown-menu border-0 shadow p-3">
                                                            <li><a class="dropdown-item py-2 rounded" href="#" onclick="view_detail({{$item->id}})" href="#" data-bs-toggle="modal" data-bs-target="#MarketCreate">View</a></li>
                                                            <li><a class="dropdown-item py-2 rounded" id="btn-edit" onclick="edit({{$item->id}})" href="#" data-bs-toggle="modal" data-bs-target="#MarketCreate">Edit</a></li>
                                                        </ul>
                                                    </div>
                                                </td>
                                            </tr>
                            ...
                            <input type="hidden" id="get-total-Mmarket" value="{{ $Mmarket->total() }}">
                            <input type="hidden" id="currentPage-Mmarket" value="1">
                            <caption class="caption-bottom">
                                <div class="md-flex-bt-i-c">
                                    <p class="py2" id="Mmarket-showingEntries">{{ showingEntriesTable($Mmarket, 'Mmarket') }}</p>
                                        <div id="Mmarket-paginate">
                                            {!! paginateTable($Mmarket, 'Mmarket') !!} <!-- ข้อมูล, ชื่อตาราง -->
                                        </div>
                                </div>
                            </caption>
                        </div>
    ...
    <script type="text/javascript" src="{{ asset('assets/helper/searchTableMasterMarket.js')}}"></script>
    <script>
