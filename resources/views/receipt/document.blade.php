...
@section('pretitle')
    <div class="container">
        <div class="row align-items-center">
            <div class="col">
                <small class="text-muted">Welcome to Log Proposal & Invoice.</small>
                <h1 class="h4 mt-1">Log Proposal & Invoice (ประวัติการแก้ไข)</h1>
            </div>
        </div>
    </div>
@endsection
        ...
        <div class="col-sm-12 col-12">
            <div class="card p-4 mb-4">
                ...
                <table class="myDataTableQuotation table table-hover align-middle mb-0" style="width:100%">
                    ...
                    <tbody>
                        @if(!empty($loginvoice))
                            @foreach ($loginvoice as $key => $item)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $item->Quotation_ID }}</td>
                                <td>{{ $item->QuotationType }}</td>
                                ...
                                <td class="text-center">
                                    @if ($item->correct == $correct)
                                        @if ($correct == 0)
                                            <a href="{{ asset($pathinvoice.$item->Quotation_ID.".pdf") }}" type="button" class="btn btn-outline-dark rounded-pill lift" target="_blank" data-toggle="tooltip" data-placement="top" title="พิมพ์เอกสาร">
                                                <i class="fa fa-print"></i>
                                            </a>
                                        @else
                                            <a href="{{ asset($pathinvoice.$item->Quotation_ID.'-'.$correctinvoice.".pdf") }}" type="button" class="btn btn-outline-dark rounded-pill lift" target="_blank" data-toggle="tooltip" data-placement="top" title="พิมพ์เอกสาร">
                                                <i class="fa fa-print"></i>
                                            </a>
                                        @endif
                                    @else
