...
<div id="content-index" class="body-header border-bottom d-flex py-3">
    <div class="container-xl">
        <div class="row align-items-center">
            ...
            <div class="col-auto">
                <a href="{{ route('harmony-debit-elexa-revenue') }}" class="bt-tg-normal">Back</a>
            </div>
        </div> <!-- .row end -->
    </div>
</div>
...
<script>
    ...
    function getTableOutstanding() 
    {
        ...
        var table = $("#myDataTableOutstanding").DataTable({
            ...
            ajax: {
                url: '/harmony-debit-get-outstanding',
                type: 'POST',
                data: {
                    year: year,
                    month: month,
    ...
    $(document).on('click', '#btn-add-multi', function () {
        ...
        if (selectedItems.length === 0) {
            ...
            if ($('#select-all-outstanding').prop('checked')) {
                ...
                jQuery.ajax({
                    type: "POST",
                    url: "{!! url('harmony-debit-select-all-elexa-outstanding') !!}",
                    datatype: "JSON",
                    data: { 
                ...
                selectedItems.forEach(function(value) {
                    ...
                    if (value != "select-all-outstanding") {
                        ...
                        jQuery.ajax({
                            type: "GET",
                            url: "{!! url('harmony-debit-select-elexa-outstanding/"+value+"') !!}",
                            datatype: "JSON",
                            async: false,
                            success: function(response) {
                                if (response.data) {
    ...
    function select_receive_payment(ele, id, amount) 
    {
        ...
        if (revenueID != "") {
            ...
            if ($('#btn-receive-' + id).val() == 0) {
                ...
                jQuery.ajax({
                    type: "GET",
                    url: "{!! url('harmony-debit-select-elexa-outstanding/"+id+"') !!}",
                    datatype: "JSON",
                    async: false,
                    success: function(response) {
                        if (response.data) {
    ...
    function btnConfirm() 
    {
        ...
        if (total_selected_amount == sms_amount) {
            ...
            jQuery.ajax({
                type: "POST",
                url: "{!! url('harmony-debit-confirm-select-elexa-outstanding') !!}",
                datatype: "JSON",
        ...
        if (total_revenue_amount == total_receive_payment) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    jQuery.ajax({
                        type: "POST",
                        url: "{!! route('harmony-debit-elexa-store') !!}",
                        datatype: "JSON",
                        data: $('#form-elexa').serialize(),
                        async: false,
                        success: function(result) {
                            // ใช้ window.location เพื่อไปยัง URL ที่ต้องการหลังจากบันทึก
                            window.location.href = "{!! route('harmony-debit-elexa-revenue') !!}";
                        },
                    });
