...
<div id="content-index" class="body-header border-bottom d-flex py-3">
    <div class="container-xl">
        <div class="row align-items-center">
            ...
            <div class="col-auto">
                <a href="{{ route('harmony-debit-agoda-revenue') }}" class="bt-tg-normal">Back</a>
            </div>
        </div> <!-- .row end -->
    </div>
</div>
...
<script>
    ...
    function select_receive_payment(ele, id, amount) {
        ...
        if (revenueID != "") {
            ...
            if ($('#btn-receive-' + id).val() == 0) {
                ...
                jQuery.ajax({
                    type: "GET",
                    url: "{!! url('harmony-debit-select-agoda-outstanding/"+id+"') !!}",
                    datatype: "JSON",
                    async: false,
    ...
    function btnConfirm() {
        ...
        jQuery.ajax({
            type: "POST",
            url: "{!! url('harmony-debit-confirm-select-agoda-outstanding') !!}",
            datatype: "JSON",
        ...
        if (total_revenue_amount == total_receive_payment) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    jQuery.ajax({
                        type: "POST",
                        url: "{!! route('harmony-debit-agoda-store') !!}",
                        datatype: "JSON",
                        ...
                        success: function(result) {
                            // Swal.fire('บันทึกข้อมูลเรียบร้อย!', '', 'success').then(() => {
                                ...
                                window.location.href = "{!! route('harmony-debit-agoda-revenue') !!}";
                            // });
                        },
                    });
