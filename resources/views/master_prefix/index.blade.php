    ...
    <div class="container">
        <div class="row align-items-center">
            <div class="col">
                ...
                <h1 class="h4 mt-1">Prename (คำนำหน้าชื่อ)</h1>
            </div>
            <div class="col-auto">
                <button type="button" class="btn btn-color-green lift btn_modal" data-bs-toggle="modal" data-bs-target="#PrenameCreate">
                    <i class="fa fa-plus"></i> เพิ่มคำนำหน้าชื่อ</button>
            </div>

            <!-- Prename Modal Center-->
            <div class="modal fade" id="PrenameCreate" tabindex="-1" aria-labelledby="PrenameModalCenterTitle"
            style="display: none;" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="PrenameModalCenterTitle">เพิ่มคำนำหน้าชื่อ</h5>
        ...
        <div class="col-auto">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu border-0 shadow p-3">
                    <li><a class="dropdown-item py-2 rounded" href="{{ route('Mprefix.index') }}">ทั้งหมด</a></li>
                    <li><a class="dropdown-item py-2 rounded" href="{{ route('Mprefix.ac', ['value' => 1]) }}">ใช้งาน</a></li>
                    <li><a class="dropdown-item py-2 rounded" href="{{ route('Mprefix.no', ['value' => 0]) }}">ปิดใช้งาน</a></li>
                </ul>
            </div>
        </div>
    </div> <!-- Row end  -->
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu border-0 shadow p-3">
                                            <li><a class="dropdown-item py-2 rounded" href="#" onclick="view_detail({{$item->id}})" href="#" data-bs-toggle="modal" data-bs-target="#PrenameCreate">ดูรายละเอียด</a></li>
                                            <li><a class="dropdown-item py-2 rounded" id="btn-edit" onclick="edit({{$item->id}})" href="#" data-bs-toggle="modal" data-bs-target="#PrenameCreate">แก้ไขรายการ</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>
...
<script>
    ...
    $('.btn_modal').on('click', function() {
        ...
        $('#comment').text('');
        $('#search_list').text('');
        $('#PrenameModalCenterTitle').text("เพิ่มคำนำหน้าชื่อ");
    ...
    $('#name_th').on('keyup', function () {
        ...
        $.ajax({
            type:   "GET",
            url:    "{!! url('/Mprefix/Mprename/search-list2/"+datakey+"') !!}",
            datatype:   "JSON",
    ...
    function edit(id) {
        ...
        document.getElementById('btn-save').disabled = false;
        $('#comment').text('');
        $('#PrenameModalCenterTitle').text("แก้ไขคำนำหน้าชื่อ");

        jQuery.ajax({
            type:   "GET",
            url:    "{!! url('/Mprefix/Mprename/edit/"+id+"') !!}",
            datatype:   "JSON",
            async:  false,
    ...
    function view_detail(id) {
        ...
        $('#PrenameModalCenterTitle').text("รายละเอียดคำนำหน้าชื่อ");
    ...
    $('#btn-save').on('click', function() {
        ...
        if (name_th != '') {
            ...
            if (module_name == "create") {
                ...
                jQuery.ajax({
                type:   "GET",
                url:    "{!! url('/Mprefix/Mprename/check-edit-name/"+id+"/"+datakey+"') !!}",
                datatype:   "JSON",
                async:  false,
                success: function(result) {
                        if (result.data) {
                            ...
                            if (module_name == "edit") {
                                jQuery.ajax({
                                type:   "GET",
                                url:    "{!! url('/Mprefix/Mprename/Mprefix_update/"+id+"/"+datakey+"/"+dataEN+"') !!}",
                                datatype:   "JSON",
                                async:  false,
    ...
    $('.btn-status').on('click', function() {
        ...
        jQuery.ajax({
        type:   "GET",
        url:    "{!! url('/Mprefix/change-Status/"+id+"') !!}",
        datatype:   "JSON",
        async:  false,
        success: function(result) {
