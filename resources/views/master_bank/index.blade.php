...
@section('pretitle')
    <div class="container">
        <div class="row align-items-center">
            <div class="col">
                <small class="text-muted">Welcome to Bank.</small>
                <h1 class="h4 mt-1">Bank (ธนาคาร)</h1>
            </div>
            <div class="col-auto">
                <button type="button" class="btn btn-primary lift btn_modal" data-bs-toggle="modal" data-bs-target="#BankCreate">
                    <i class="fa fa-plus"></i> เพิ่มธนาคาร</button>
            </div>

            <!-- Prename Modal Center-->
            <div class="modal fade" id="BankCreate" tabindex="-1" aria-labelledby="PrenameModalCenterTitle"
            style="display: none;" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="PrenameModalCenterTitle">เพิ่มธนาคาร</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="col-12">
                                    <div class="card-body">
                                        <form action="{{route('Mprefix.save')}}" method="POST" enctype="multipart/form-data" class="row g-3 basic-form" id="form-id">
                                            @csrf
                                            ...
                                            <div class="col-sm-12 col-12">
                                                <label class="form-label">swiftcode</label>
        ...
        <div class="col-auto">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu border-0 shadow p-3">
                    <li><a class="dropdown-item py-2 rounded" href="{{ route('Mbank.index') }}">ทั้งหมด</a></li>
                    <li><a class="dropdown-item py-2 rounded" href="{{ route('Mbank.ac', ['value' => 1]) }}">ใช้งาน</a></li>
                    <li><a class="dropdown-item py-2 rounded" href="{{ route('Mbank.no', ['value' => 0]) }}">ปิดใช้งาน</a></li>
                </ul>
            </div>
        </div>
    </div> <!-- Row end  -->
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu border-0 shadow p-3">
                                            <li><a class="dropdown-item py-2 rounded" href="#" onclick="view_detail({{$item->id}})" href="#" data-bs-toggle="modal" data-bs-target="#BankCreate">ดูรายละเอียด</a></li>
                                            <li><a class="dropdown-item py-2 rounded" id="btn-edit" onclick="edit({{$item->id}})" href="#" data-bs-toggle="modal" data-bs-target="#BankCreate">แก้ไขรายการ</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>
...
<script>
    ...
    $('.btn_modal').on('click', function() {
        ...
        $('#comment').text('');
        $('#search_list').text('');
        $('#PrenameModalCenterTitle').text("เพิ่มธนาคาร");
    ...
    $('#name_th').on('keyup', function () {
        ...
        $.ajax({
            type:   "GET",
            url:    "{!! url('/Mbank/search-list2/"+datakey+"') !!}",
            datatype:   "JSON",
            success: function(data) {
                if (data.name_th) {
                    console.log(data.name_th);
                    $('#comment').text("** '" + data.name_th + "' มีอยูในระบบแล้ว !");
                    $('#search_list').text('มีคำนำหน้าชื่อซ้ำกันแล้ว');
    ...
    function edit(id) {
        ...
        document.getElementById('btn-save').disabled = false;
        $('#comment').text('');
        $('#PrenameModalCenterTitle').text("แก้ไขธนาคาร");

        jQuery.ajax({
            type:   "GET",
            url:    "{!! url('/Mbank/edit/"+id+"') !!}",
            datatype:   "JSON",
            async:  false,
            success: function(result) {
                ...
                $('#swiftcode').val(result.data.swiftcode);
            },
        });
    }
    ...
    function view_detail(id) {
        ...
        $('#PrenameModalCenterTitle').text("รายละเอียดธนาคาร");
            ...
            if (module_name == "create") {
                ...
                success: function(data) {
                        if (data.name_th) {
                            console.log(data.name_th);
                            $('#comment').text("** '" + data.name_th + "' มีอยูในระบบแล้ว !");
                            $('#search_list').text('มีธนาคารซ้ำกันแล้ว');
                            document.getElementById('btn-save').disabled = true;
                        }else{
                            $('#form-id').submit();
                        }
                ...
                jQuery.ajax({
                type:   "GET",
                url:    "{!! url('/Mbank/check-edit-name/"+id+"/"+datakey+"') !!}",
                datatype:   "JSON",
                async:  false,
                success: function(result) {
                        if (result.data &&
                            ...
                            result.data.swiftcode === swiftcode) {
                            $('#comment').text("** '" + result.data.name_th + "', '" + result.data.name_en + "', '" + result.data.code + "', '" + result.data.swiftcode + "' มีอยูในระบบแล้ว !");
                            document.getElementById('btn-save').disabled = true;
                        }else{
                            if (module_name == "edit") {
                                jQuery.ajax({
                                type:   "GET",
                                url:    "{!! url('/Mbank/update/"+id+"/"+datakey+"/"+dataEN+"/"+code+"/"+swiftcode+"') !!}",
                                datatype:   "JSON",
                                async:  false,
    ...
    $('.btn-status').on('click', function() {
        ...
        jQuery.ajax({
        type:   "GET",
        url:    "{!! url('/Mbank/change-Status/"+id+"') !!}",
        datatype:   "JSON",
        async:  false,
        success: function(result) {
