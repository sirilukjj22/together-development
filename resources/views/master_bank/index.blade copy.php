    ...
    <div class="container-fluid border rounded-3 p-5 mt-3 bg-white" style="width: 98%;">
        ...
        <div class="col-lg-12" style="float: right">
            <div  class="col-lg-4" style="float: right">
                <button type="button" class="create"  data-bs-toggle="modal" data-bs-target="#MbankCreate">
                    + Add Master Bank
                </button>
            </div>
        </div>
        <div  class="col-4 mt-3 statusbtndiv">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <a class="dropdown-item" href="{{ route('Mbank.index') }}">ทั้งหมด</a>
                <a class="dropdown-item" style="color: green;" href="{{ route('Mbank.ac', ['value' => 1]) }}">เปิดใช้งาน</a>
                <a class="dropdown-item" style="color: #f44336;" href="{{ route('Mbank.no', ['value' => 0]) }}">ปิดใช้งาน</a>
            </div>
        </div>
                    ...
                    @if (!empty($Mbank))
                        @foreach ($Mbank as $key => $item)
                            <tr>
                                ...
                                <td data-label="ชื่อผู้ใช้งาน">{{ $item->name_en }}</td>
                                ...
                                <td>
                                    <div class="dropdown-a">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li class="licolor"><a  data-bs-toggle="modal" data-bs-target="#Mbank{{$item->id}}">แก้ไขข้อมูล</a></li>
                                        </ul>
                                </td>
                            </tr>
                        @endforeach
        ...
        @foreach ($Mbank as $item)
            <div class="modal fade" id="Mbank{{$item->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog  modal-lg">
                <div class="modal-content">
                    <div class="modal-header"style=" background-color: #2D7F7B;">
                        <h1 class="modal-title fs-5" id="staticBackdropLabel" style=" color: #FFFFFF;">Edit Master Bank</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" style=" background-color: #FFFFFF;" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ url('/Mbank/master_bank/Mbank_update/') }}" method="POST"enctype="multipart/form-data">
                            @csrf
                            <div class="col-12 ">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-6">
                                        ...
                                        <input type="text" id="swiftcode" name="swiftcode" value="{{$item->swiftcode}}">
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-6">
        ...
        @endforeach
        <div class="modal fade" id="MbankCreate" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog  modal-lg">
            <div class="modal-content">
                <div class="modal-header"style=" background-color: #2D7F7B;">
                <h1 class="modal-title fs-5" id="staticBackdropLabel" style=" color: #FFFFFF;">+ Add Master Bank</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" style=" background-color: #FFFFFF;" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{route('Mbank.save')}}" method="POST"enctype="multipart/form-data">
                        @csrf
                        <div class="col-12 ">
                            <div class="row">
                                <div class="col-6">
...
$(document).ready(function() {
    $('.status-toggle').click(function() {
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/Mbank/change-Status/') }}" + '/' + id + '/' + status,
