...
@section('content')
        ...
        <div class="container-xl">
                        ...
                        <input type="text" id="select-date" name="" placeholder="{{ !empty($pickup_time) ? $pickup_time : date('d F Y') }}" value="{{ $pickup_time }}" readonly>
                            ...
                            <span class="dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuDaily">
                                    <a class="dropdown-item" href="{{ route('harmony-revenue') }}">Today</a>
            ...
            <div class="table-2" style="overflow-x:auto;">
                <div class="d-flex gap-3 mb-2">
                    <div class="center" >
                        <img src="{{ asset('assets/images/harmony/logo_2.png') }}" alt="logo of Harmony Resort" width="80" class="mb-1" />
                    </div>
      
                    <div class="text-capitalize d-grid gap-0">
                      <span class="f-semi">Harmony Resort</span>
    ...
    <div class="modal fade" id="ModalShowCalendar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-bottom md-330px" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('harmony-revenue-search-calendar') }}" method="POST" enctype="multipart/form-data" class="" id="form-revenue">
                    @csrf
    ...
    <div class="modal fade bd-example-modal-lg" id="addIncome" tabindex="-1" role="dialog" aria-labelledby="addIncomeLabel" aria-hidden="true">
...
<script>
    ...
    function Add_data($date) {
        ...
        jQuery.ajax({
            type:   "GET",
            url:    "{!! url('harmony-revenue-edit/"+date+"') !!}",
            datatype:   "JSON",
            async:  false,
    ...
    $('.btn-close-daily').on('click', function () {
        ...
        if (result.isConfirmed) { 
            $.ajax({
                url: "{!! url('harmony-revenue-daily-close') !!}",
                type: 'POST',
                dataType: "json",
                cache: false,
                data: {
    ...
    $('.btn-open-daily').on('click', function () {
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "{!! url('harmony-revenue-daily-open') !!}",
                type: 'POST',
                dataType: "json",
                cache: false,
                data: {
    ...
    function revenue_store() {
        jQuery.ajax({
            url:    "{!! route('harmony-revenue-store') !!}",
            type: 'POST',
            dataType: "json",
            cache: false,
