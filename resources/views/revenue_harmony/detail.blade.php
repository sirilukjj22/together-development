...
@section('content')
                    ...
                    <div class="card p-4 mb-4">
                        ...
                        <caption class="caption-bottom">
                            <div class="md-flex-bt-i-c">
                                <p class="py2" id="revenue-showingEntries">{{ showingEntriesTable($data_query, 'revenue') }}</p>
                                <div class="font-bold ">ยอดรวมทั้งหมด {{ number_format($total_query, 2) }} บาท</div>
                                    <div id="revenue-paginate">
                                        {!! paginateTable($data_query, 'revenue') !!} <!-- ข้อมูล, ชื่อตาราง -->
                                    </div>
                            </div>
                        </caption>
                    </div> <!-- .card end -->
    ...
    <input type="hidden" id="get-total-revenue" value="{{ $data_query->total() }}">
    ...
    <script type="text/javascript" src="{{ asset('assets/helper/searchTableRevenueDetailHarmony.js')}}"></script>
    ...
    <script>
        ...
        $(document).on('keyup', '.search-data', function () {
            ...
            var table = $('#'+table_name).dataTable({
                    ...
                    info: false,
                    ajax: {
                        url: '/harmony-' + getUrl +'-search-table',
                        type: 'POST',
                        dataType: "json",
                        cache: false,
                        data: {
